...
const mockNotes: HandoverNote[] = [
  {
    ...
    transcript:
      'Morning shift handover. Line 1 running smoothly at 85% OEE. Minor issue with Filler-2 temperature running slightly high at 71 degrees. Maintenance team notified. Quality checks all passing. Next shift should monitor temperature closely.',
    summary:
      'Line 1 operational at 85% OEE. Filler-2 temperature slightly elevated (71°C), maintenance aware. Quality metrics good.',
    actionItems: [
      {
        id: 'action-1',
  ...
  },
  {
    ...
    transcript:
      'Afternoon shift update for Line 2. Production rate is lower than target due to belt alignment issue on Assembly-1. Technician adjusted tension but recommend full inspection. Completed 850 units today, target was 1000. No quality issues reported.',
    summary:
      'Line 2 below target (850/1000 units) due to belt alignment issue on Assembly-1. Needs full inspection.',
    actionItems: [
      {
        id: 'action-3',
...
export class HandoverClient {
  ...
  private generateLocalSummary(
    ...
    const qualityGood = lower.match(
      /quality\s+(?:checks?|metrics?)?\s+(?:are|is)?\s*(?:all\s+)?(?:passing|good|normal|acceptable)/i
    );
  ...
  private buildKnowledgeContent(note: HandoverNote): string {
    ...
    sections.push('---');
    sections.push(
      '*This knowledge article was automatically generated from a shift handover recording.*'
    );
